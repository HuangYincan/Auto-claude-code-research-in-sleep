...
- What data do we have (vector layers, raster layers)?
...
Use `ping` to verify the connection, then gather context with `get_qgis_info`
and (if a project is loaded) `get_project_info`.
...
Combine with other ARIS skills (e.g. `/analyze-results`, `/geo-awareness`,
`/geo-mapping`) as needed for research workflows.
